...
for i=0:41
    ...
    date
    cluster_density=flipud(csvread(strcat(date,'StandAngCount.csv')));
    imagesc(cluster_density,[0,20])
    title1=strcat('Still Customer Distribution on 2',date(2:end),Week_pool(i+1));
    title(title1)
    colorbar
    drawnow
    frame = getframe(1);
    ...
    if(i==0)
        imwrite(A,map,'StandCount.gif','DelayTime',0.5,'LoopCount',Inf)
    else
        imwrite(A,map,'StandCount.gif','WriteMode','append','DelayTime',0.5)    
    end
