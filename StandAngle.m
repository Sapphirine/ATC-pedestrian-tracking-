...
for i=0:41
    ...
    date
    cluster_density=flipud(csvread(strcat(date,'StandAvgAngle.csv')));
    imagesc(cluster_density)
    title1=strcat('Customer Stand Facing Direction on 2',date(2:end),Week_pool(i+1));
    title(title1)
    drawnow
    frame = getframe(1);
    im = frame2im(frame);
    ...
    if(i==0)
        imwrite(A,map,'StandFacing.gif','DelayTime',0.5,'LoopCount',Inf)
    else
        imwrite(A,map,'StandFacing.gif','WriteMode','append','DelayTime',0.5)    
    end
